...
img:asm hash kernelSize
	...
	dd if=$(BUILD_DIR)/loader.bin of=hd60M.img bs=512 count=34 seek=1 conv=notrunc
	dd if=$(BUILD_DIR)/kernel.bin of=hd60M.img bs=512 count=200 seek=35 conv=notrunc
	dd if=hash of=hd60M.img bs=512 count=1 seek=52 conv=notrunc
	dd if=kernelSize of=hd60M.img bs=512 count=1 seek=53 conv=notrunc
